    ...
    <div className="col" style={{ gap: 20 }}>
      ...
      <div className="grid-2">
        ...
        <div className="field">
          ...
          <div className="toggle-row">
            <button className={`toggle-pill ${s.isMarried === true ? 'on' : ''}`} onClick={() => s.set('isMarried', true)}>결혼했어요</button>
            <button className={`toggle-pill ${s.isMarried === false ? 'on' : ''}`} onClick={() => s.set('isMarried', false)}>미혼이에요</button>
          </div>
        </div>
      </div>

      {/* 결혼했을 때만 신혼/맞벌이 질문 노출 */}
      <AnimatePresence>
        {s.isMarried === true && (
          <motion.div
          ...
          >
            <div className="col" style={{ gap: 20 }}>
              <div className="field">
                ...
                <div className="toggle-row">
                  <button className={`toggle-pill ${s.isNewCouple === true ? 'on' : ''}`} onClick={() => s.set('isNewCouple', true)}>신혼이에요</button>
                  <button className={`toggle-pill ${s.isNewCouple === false ? 'on' : ''}`} onClick={() => s.set('isNewCouple', false)}>아니에요</button>
                </div>
              </div>
              <div className="field">
                <label className="field-label">맞벌이 여부 (신생아특례 자격 판정)</label>
                <div className="toggle-row">
                  <button className={`toggle-pill ${s.isDualIncome === true ? 'on' : ''}`} onClick={() => s.set('isDualIncome', true)}>맞벌이</button>
                  <button className={`toggle-pill ${s.isDualIncome === false ? 'on' : ''}`} onClick={() => s.set('isDualIncome', false)}>외벌이</button>
                </div>
              </div>
            </div>
          </motion.div>
      ...
      <div className="grid-2">
        <div className="field">
          ...
          <div className="toggle-row">
            <button className={`toggle-pill ${s.isFirstTime === true ? 'on' : ''}`} onClick={() => s.set('isFirstTime', true)}>처음이에요</button>
            <button className={`toggle-pill ${s.isFirstTime === false ? 'on' : ''}`} onClick={() => s.set('isFirstTime', false)}>아니에요</button>
          </div>
        </div>
        <div className="field">
          <label className="field-label">현재 무주택</label>
          <div className="toggle-row">
            <button className={`toggle-pill ${s.isNoHouse === true ? 'on' : ''}`} onClick={() => s.set('isNoHouse', true)}>무주택</button>
            <button className={`toggle-pill ${s.isNoHouse === false ? 'on' : ''}`} onClick={() => s.set('isNoHouse', false)}>유주택</button>
          </div>
        </div>
      </div>
      ...
      <div className="field">
        ...
        <div className="toggle-row">
          <button className={`toggle-pill ${s.hasNewBorn2yr === true ? 'on' : ''}`} onClick={() => s.set('hasNewBorn2yr', true)}>출산했어요</button>
          <button className={`toggle-pill ${s.hasNewBorn2yr === false ? 'on' : ''}`} onClick={() => s.set('hasNewBorn2yr', false)}>아니에요</button>
        </div>
      </div>
      ...
      <div className="field">
        ...
        <input
          ...
          onChange={(e) => {
            ...
            s.set('kidsCount', n);
            s.set('isHavingKids', n >= 1);
          }}
        />
      </div>

      <div className="field">
        <label className="field-label">우대금리 해당 사항 (복수 선택 가능)</label>
        <div className="toggle-row">
          <button className={`toggle-pill ${s.isSingleParent === true ? 'on' : ''}`} onClick={() => s.set('isSingleParent', !s.isSingleParent)}>한부모</button>
          <button className={`toggle-pill ${s.isMultiCultural === true ? 'on' : ''}`} onClick={() => s.set('isMultiCultural', !s.isMultiCultural)}>다문화</button>
          <button className={`toggle-pill ${s.isDisabled === true ? 'on' : ''}`} onClick={() => s.set('isDisabled', !s.isDisabled)}>장애인</button>
        </div>
      </div>
    </div>
  );
      ...
      <div className="field">
        ...
        <div className="field-wrap">
          <input
            ...
            onChange={(e) => {
              const val = e.target.value;
              s.set('yearIncome', val ? parseInt(val, 10) : ('' as any));
            }}
            placeholder="예: 9000"
          />
          <span className="field-suffix">만원 / 년</span>
        </div>
        {!!s.yearIncome && <div className="t-mute">{comma(Number(s.yearIncome))} 만원 = {(Number(s.yearIncome) / 10000).toFixed(2)} 억</div>}
      </div>
      ...
      <div className="field">
        ...
        <div className="field-wrap">
          <input
            ...
            onChange={(e) => {
              const val = e.target.value;
              s.set('myAsset', val ? parseInt(val, 10) : ('' as any));
            }}
            placeholder="예: 30000"
          />
          <span className="field-suffix">만원</span>
        </div>
        {!!s.myAsset && <div className="t-mute">{comma(Number(s.myAsset))} 만원 = {(Number(s.myAsset) / 10000).toFixed(2)} 억</div>}
      </div>
      ...
      <div className="field">
        ...
        <div className="field-wrap">
          <input
            ...
            onChange={(e) => {
              const val = e.target.value;
              s.set('monthlySaving', val ? parseInt(val, 10) : ('' as any));
            }}
            placeholder="예: 200"
          />
